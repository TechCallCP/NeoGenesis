...
public class AssembleShipCommand {
    ...
    private static final Integer MAX_BLOCKS = Integer.valueOf(256);
    ...
    private static Integer execute(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        ShipAssembler.INSTANCE.assembleToShip(level, blocks, Boolean.TRUE, Double.valueOf(1.0D), Boolean.FALSE);
        ctx.getSource().sendSuccess(
                () -> Component.literal("Assembled " + blocks.size() + " blocks into a ship at " + origin),
                Boolean.FALSE);
        return Integer.valueOf(blocks.size());
    }
    ...
    private static List<BlockPos> collectBlocks(ServerLevel level, BlockPos origin) {
        ...
        while (!queue.isEmpty() && result.size() < MAX_BLOCKS.intValue()) {
